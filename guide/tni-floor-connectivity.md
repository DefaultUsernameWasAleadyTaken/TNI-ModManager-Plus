...
**Новичок / день 1 + расширение (бэкапы, jailbreak, VLAN, RIP, Git…):** отдельный пошаговый гайд → [`tni-day1-starter.md`](./tni-day1-starter.md).

Содержание дальше по файлу: режимы A/B → **несколько ЦОД / рост и ПС / цвета кабелей** → пошагово → как работает сеть → алиасы → чеклисты → бэкапы/Morris → **Registry/ISP, телефоны/камеры, VLAN, RIP, pcap, blackhole, таблица портов, Tower Link**.
...
В ЦОД **обязательно** держи корневой DNS+DHCP (`@c1/dns` / `@c1/dhcp`) для имён ядра и money. DHCP **блока** живёт на f1 блока (`@c1/bN/dhcp`); клиенты блока через DHCP получают DNS блока **и** `@c1/dns`. Этажи f2/f3 — свой `dns-lite`. Подробный day-1: [`tni-day1-starter.md`](tni-day1-starter.md).
...
Длины меряй **T**. В магазине: **один цвет = один заказ на одну длину** (нельзя в одном заказе смешать синий и красный одной длины). Плагины питания: после одного заказа можно сразу открыть второй — склад «обновляется».

### Цвета кабелей — какой лучше

**Скорость от цвета не зависит.** Цвет нужен, чтобы в спагетти ЦОД за секунду понять роль линка и не выдернуть money вместо патча к клиенту. Выбери схему **в день 1** и не меняй mid-run.

Рекомендуемая схема под этот гайд (copper Ethernet):

| Цвет | Роль | Типичные длины |
|------|------|----------------|
| **Синий** | Клиенты этажа → switch (consumer patches) | 200–500 |
| **Зелёный** | Продюсеры / телефон / камера → switch | 200–500 |
| **Жёлтый** | Uplink этажа: switch → riser (Tower Link сторона) | 500–2000 |
| **Оранжевый** | Роутер↔роутер: блок → ядро, ядро → svc, **ЦОД↔ЦОД** | 200–500 |
| **Красный** | Money / критичное: DNS, VOIP, Git, Padu, NAS | 200–500 |
| **Фиолетовый** (или белый) | Только debugger | 1000–1500 |
| **Серый** | Временное / «не знаю куда» — потом перекрась ролью | любые |

Питание — **отдельные** UK plug / Tenabolt, не путай с Ethernet.

Почему так: жёлтый uplink и оранжевый «ствол» сразу видны на фоне синих патчей; красный не трогаешь без нужды при аварии; фиолетовый debugger не спутаешь с боем. Если в UI мало цветов — минимум **три**: патчи клиентов / uplink этажа / всё межроутерное+серверы.

Fiber (когда дойдёшь): тот же принцип по ролям; для длинных стволов ЦОД↔ЦОД и толстого блок→ядро fiber часто выгоднее по ПС/дневке, чем топовый copper — сверяй таблицу cat/OM в главе Tower Link.

### Закупка A (старт)

| Роль | Что брать | Цвет (по схеме выше) |
|------|-----------|----------------------|
| Edge + ядро | Disco Milli + Disco Micro | — |
| DNS (+ VOIP/Git) | Boulder+ ×2–3 | — |
| Этаж | Blade5 × число этажей, патчи | синий (+ зелёный под phone/cam) |
| Uplink этажа | 500–2000 | **жёлтый** |
| DC короткие линки | Router/Server 200–500 | оранжевый (роутеры), **красный** (к серверам) |
| Питание | UK plug / Tenabolt | — |
| Debugger | длинный Ethernet | фиолетовый / белый |
